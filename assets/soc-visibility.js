(() => {
 ...
 function draw(){
  ...
  ctx.strokeStyle="rgba(105,231,255,.08)";ctx.lineWidth=1;
  for(let y=18;y<h;y+=34){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke()}
  for(let x=0;x<w;x+=56){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,h);ctx.stroke()}
  const d=datasets[mode];line(d.inbound,"#69e7ff",w,h);line(d.outbound,"#7fffc1",w,h);
  phase+=.025;raf=requestAnimationFrame(draw)
 }
 function select(next){
  mode=next;tabs.forEach(x=>x.classList.toggle("active",x.dataset.socView===next));
  panels.forEach(x=>x.hidden=x.dataset.socPanel!==next);
  metric.textContent=next==="controls"?"98.6%":next==="firewalls"?"2.4 Gb/s":(640+Math.round(Math.random()*190))+" Mb/s";
 }
 tabs.forEach(x=>x.addEventListener("click",()=>select(x.dataset.socView)));
 refresh.addEventListener("click",()=>{refresh.classList.add("spin");root.querySelector("[data-soc-time]").textContent="SYNCHRONIZING COLLECTORS";setTimeout(()=>{refresh.classList.remove("spin");root.querySelector("[data-soc-time]").textContent="COLLECTORS UPDATED · JUST NOW";select(mode)},650)});
 new ResizeObserver(size).observe(canvas);size();select(mode);draw();
