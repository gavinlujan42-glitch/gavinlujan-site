(() => {
  ...
  const output=root.querySelector('[data-mantra-text]'),discipline=root.querySelector('[data-mantra-discipline]'),counter=root.querySelector('[data-mantra-counter]'),toggle=root.querySelector('[data-mantra-toggle]'),previous=root.querySelector('[data-mantra-prev]'),following=root.querySelector('[data-mantra-next]'),reduceMotion=window.matchMedia('(prefers-reduced-motion: reduce)');
  ...
  const status=root.querySelector('.mantra-status span:last-child'); if(status)status.textContent='60 SEC MAIN SCREEN';
